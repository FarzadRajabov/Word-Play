...
export default function AntonymChallenge() {
  ...
  useEffect(() => {
    if (isLoaded && words.length >= OPTIONS_COUNT) {
      const wordsWithAntonyms = words.filter((word) => word.antonyms.length > 0)
      if (wordsWithAntonyms.length >= GAME_SIZE) {
        startGame(wordsWithAntonyms)
      }
    }
  }, [isLoaded])

  const startGame = (wordsWithAntonyms: Word[]) => {
    const selectedWords = wordsWithAntonyms.sort(() => 0.5 - Math.random()).slice(0, GAME_SIZE)
  ...
  const generateOptions = (gameWords: Word[], index: number) => {
    ...
    // Get a random antonym as the correct answer
    const correctAntonym = currentWord.antonyms[Math.floor(Math.random() * currentWord.antonyms.length)]
    const correctOption = { term: correctAntonym, isCorrect: true }

    // Get other random words for incorrect options
    const otherWords = words
      .filter((w) => w.id !== currentWord.id && !currentWord.antonyms.includes(w.term))
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        ...
        <Card className="max-w-md mx-auto">
          <CardHeader>
            <CardTitle>Not Enough Words with Antonyms</CardTitle>
            <CardDescription>
              You need at least {GAME_SIZE} words with antonyms to play Antonym Challenge.
            </CardDescription>
          </CardHeader>
          <CardFooter>
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      {!gameOver ? (
        <Card className="max-w-2xl mx-auto">
          <CardHeader>
            <CardTitle className="text-center">Antonym Challenge</CardTitle>
            <CardDescription className="text-center">Choose the word with the opposite meaning</CardDescription>
          ...
          <CardContent className="space-y-6">
            <div className="bg-primary/10 p-6 rounded-lg text-center">
              ...
              <p className="mt-2 text-muted-foreground">{gameWords[currentWordIndex]?.definition}</p>
            </div>
        ...
        <Card className="max-w-md mx-auto">
          ...
          <CardFooter className="flex justify-between">
            ...
            <Button onClick={() => startGame(wordsWithAntonyms)}>Play Again</Button>
          </CardFooter>
        </Card>
      )}
    </div>
