...
export default function AntonymChallenge() {
  ...
  useEffect(() => {
    if (isLoaded && words.length >= OPTIONS_COUNT) {
      const wordsWithAntonyms = words.filter(
        (word) => word.antonyms.length > 0
      );
  ...
  const startGame = (wordsWithAntonyms: Word[]) => {
    const selectedWords = wordsWithAntonyms
  ...
  const generateOptions = (gameWords: Word[], index: number) => {
    ...
    const correctAntonym =
      currentWord.antonyms[
        Math.floor(Math.random() * currentWord.antonyms.length)
      ];
    ...
    const otherWords = words
      .filter(
        (w) => w.id !== currentWord.id && !currentWord.antonyms.includes(w.term)
      )
      .sort(() => 0.5 - Math.random())
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <Card className="max-w-md mx-auto">
          <CardHeader>
            ...
            <CardDescription>
              You need at least {GAME_SIZE} words with antonyms to play Antonym
              Challenge.
            </CardDescription>
          </CardHeader>
          <CardFooter>
      ...
      {!gameOver ? (
          ...
          <CardContent className="space-y-6">
            <div className="bg-primary/10 p-6 rounded-lg text-center">
              ...
              <p className="mt-2 text-muted-foreground">
                {gameWords[currentWordIndex]?.definition}
              </p>
            </div>
        ...
        <Card className="max-w-md mx-auto">
          ...
          <CardFooter className="flex justify-between">
            ...
            <Button onClick={() => startGame(wordsWithAntonyms)}>
              Play Again
            </Button>
          </CardFooter>
        </Card>
